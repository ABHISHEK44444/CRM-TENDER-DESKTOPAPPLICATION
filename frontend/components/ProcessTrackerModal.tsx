...
// FIX: Removed MOCK_USERS import as users will be passed via props.
...
interface ProcessTrackerModalProps {
    tender: Tender;
    currentUser: User;
    // FIX: Added 'users' prop to provide user data to the component.
    users: User[];
    onClose: () => void;
...
// FIX: Add constant for stage status options to resolve compilation error.
                                ...
                                <div className="p-4 bg-white dark:bg-[#161b22] border border-slate-200 dark:border-[#30363d] rounded-lg shadow-sm">
                                    ...
                                    {stageData.updatedAt && (
                                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                            {/* FIX: Type 'unknown' is not assignable to type 'ReactNode'. Use the userMap to get the name string. */}
                                            Last updated by {userMap.get(stageData.updatedById || '') || 'N/A'} on {new Date(stageData.updatedAt).toLocaleDateString()}
                                        </p>
                                    )}
