...
const WorkflowChecklist: React.FC<WorkflowChecklistProps> = ({ tender, currentUser, onUpdateTender }) => {
    ...
    const actualChecklistItems = currentChecklist.filter(item => !item.id.includes('heading-'));
    const checklistProgress = actualChecklistItems.length > 0
        ? (actualChecklistItems.filter(i => i.completed).length / actualChecklistItems.length) * 100
        : 0;
                ...
                <div className="space-y-3">
                    ...
                    {currentChecklist.map(item => {
                        if (item.id.includes('heading-')) {
                            return <h4 key={item.id} className="text-sm font-semibold text-slate-600 dark:text-slate-400 mt-3 pt-2 border-t border-slate-200 dark:border-slate-700">{item.text}</h4>;
                        }
                        return (
