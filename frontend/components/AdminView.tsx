...
import { PlusCircleIcon, PencilIcon, TrashIcon, SearchIcon, EyeIcon, EyeOffIcon } from '../constants';
...
const AdminView: React.FC<AdminViewProps> = (props) => {
    const { users, onAddUser, onEditUser, onUpdateUserStatus, onDeleteUser, currentUser, departments, designations, onSaveDepartment, onDeleteDepartment, onSaveDesignation, onDeleteDesignation, biddingTemplates, onSaveTemplate, onDeleteTemplate, products, onAddOrUpdateProduct, onEditProduct, onDeleteProduct } = props;
                                    ...
                                    {filteredUsers.map((user) => (
                                        <tr key={user.id} className="bg-white dark:bg-slate-800 border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-center space-x-4">
                                                ...
                                                {user.status === 'Active' ? (
                                                    <button onClick={() => onUpdateUserStatus(user.id, 'Inactive')} className="font-medium text-yellow-600 dark:text-yellow-500 hover:underline">
                                                        Deactivate
                                                    </button>
                                                ) : (
                                                    <button onClick={() => onUpdateUserStatus(user.id, 'Active')} className="font-medium text-green-600 dark:text-green-500 hover:underline">
                                                        Activate
                                                    </button>
                                                )}
