    ...
    if (isProcessing) {
        return (
             <div className="flex flex-col items-center justify-center h-full text-slate-500 dark:text-slate-400 py-20">
                ...
                <p className="mt-4 text-lg font-semibold">AI is analyzing your document</p>
                <p className="text-sm mt-1 w-full text-center min-h-[20px]">{loadingMessage}</p>
            </div>
        );
    }
    ...
    if (extractedData) {
        return (
             <div className="p-6 overflow-y-auto flex-grow space-y-6">
                ...
                <div>
                     <h4 className="text-md font-semibold text-slate-700 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 pb-2 mb-4">Financial & Performance Requirements</h4>
                      <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <FormInput label="Past Performance" name="pastPerformance" value={extractedData.pastPerformance} onChange={handleDataChange} type="text"/>
                        </div>
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                     <h4 className="text-md font-semibold text-slate-700 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 pb-2 mb-4">Exemptions & Seller Documents</h4>
                      <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">MSE Exemption</label>
                                <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-md p-2 text-slate-800 dark:text-slate-200 font-medium">
                                    {extractedData.mseExemption === undefined || extractedData.mseExemption === null ? 'N/A' : (extractedData.mseExemption ? 'Yes' : 'No')}
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Startup Exemption</label>
                                <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-md p-2 text-slate-800 dark:text-slate-200 font-medium">
                                    {extractedData.startupExemption === undefined || extractedData.startupExemption === null ? 'N/A' : (extractedData.startupExemption ? 'Yes' : 'No')}
                                </div>
                            </div>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Documents Required from Seller</label>
                            {(extractedData.sellerRequiredDocuments && extractedData.sellerRequiredDocuments.length > 0) ? (
                                <ul className="list-disc list-inside bg-slate-100 dark:bg-slate-700 rounded-md p-3 text-slate-800 dark:text-slate-200 space-y-1 text-sm">
                                    {extractedData.sellerRequiredDocuments.map((doc, index) => (
                                        <li key={index}>{doc}</li>
                                    ))}
                                </ul>
                            ) : (
                                <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-md p-2 text-slate-500 dark:text-slate-400">
