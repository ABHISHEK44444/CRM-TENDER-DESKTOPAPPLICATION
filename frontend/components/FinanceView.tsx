...
const FinanceDashboardTab: React.FC<Pick<FinanceViewProps, 'tenders' | 'financialRequests' | 'currentUser' | 'users'>> = ({ tenders, financialRequests, users }) => {
...
const FinanceRequestsTab: React.FC<FinanceViewProps> = ({ tenders, financialRequests, currentUser, users, onRequestNew, onUpdateRequestStatus, onProcessRequest, onDeclineRequest }) => {
                ...
                <table className="w-full text-sm text-left text-slate-500 dark:text-slate-400">
                     ...
                     <tbody>
                        {filteredRequests.map(req => {
                            ...
                            let expiryDate: string | undefined | null = null;
                            if (req.type.startsWith('EMD')) {
                                expiryDate = tender?.emd?.expiryDate;
                            } else if (req.type === FinancialRequestType.PBG) {
                                expiryDate = req.instrumentDetails?.expiryDate;
                            }
