...
const UserFormModal: React.FC<UserFormModalProps> = ({ onClose, onSave, user, departments, designations }) => {
  ...
  const validate = () => {
    ...
    if (!isEditing) {
        ...
        if (formData.password) { // If admin is changing password
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validate()) {
      const dataToSave = { ...formData, department: formData.department || undefined, designation: formData.designation || undefined };
      if (user) { // Editing
        const specializationsArray = dataToSave.specializations ? dataToSave.specializations.split(',').map(s => s.trim()).filter(Boolean) : [];
        const updatedUser: User = {
          ...user, // keep id, status etc.
        ...
        if (dataToSave.password) {
          updatedUser.password = dataToSave.password;
        }
        onSave(updatedUser);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           ...
           <div>
            ...
            <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 mb-2">{isEditing ? 'Change Password (optional)' : 'Set Password'}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
