...
const NotificationDropdown: React.FC<NotificationDropdownProps> = ({ notifications, onClose, onNotificationClick, onMarkAsRead, onViewAll }) => {
  useEffect(() => {
    // When the dropdown opens, find unread notifications and mark them as read.
    const unreadIds = notifications.filter(n => !n.isRead).map(n => n.id);
    if (unreadIds.length > 0) {
        onMarkAsRead(unreadIds);
    }
  // NOTE: This effect runs when the list of notifications changes, ensuring we mark new ones as read.
  // The onMarkAsRead callback is memoized in the parent, preventing loops.
