      ...
      <div className="flex-grow grid grid-cols-1 md:grid-cols-4 gap-8 overflow-hidden">
        ...
        <div className="md:col-span-1 bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-4 overflow-y-auto">
          ...
          <nav className="space-y-1">
            {allStages.map(stage => {
              const totalTasks = (STANDARD_WORKFLOW_CHECKLISTS[stage] || []).filter(i => !i.id.startsWith('heading-')).length;
              const completedTasks = (standardProcessState[stage] || []).filter(id => !id.startsWith('heading-')).length;
            ...
            {checklist.length > 0 ? (
              checklist.map((item) => {
                if (item.id.startsWith('heading-')) {
                    return <h4 key={item.id} className="text-lg font-semibold text-slate-800 dark:text-slate-200 mt-6 mb-2 pt-2 border-t border-slate-200 dark:border-slate-700">{item.text}</h4>;
                }
