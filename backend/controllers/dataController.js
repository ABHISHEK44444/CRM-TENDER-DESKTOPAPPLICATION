...
const getStandardProcessState = async (req, res) => {
    try {
        let processState = await StandardProcess.findOne({ identifier: 'main' });
        if (!processState) {
            processState = await StandardProcess.create({ identifier: 'main', state: {} });
        }
        res.status(200).json(processState.state);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};

const updateStandardProcessState = async (req, res) => {
    try {
        const newState = req.body;
        const updatedProcessState = await StandardProcess.findOneAndUpdate(
            { identifier: 'main' },
            { state: newState },
            { new: true, upsert: true, runValidators: true }
        );
        res.status(200).json(updatedProcessState.state);
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
};
...
module.exports = {
    ...
    saveTemplate,
    deleteTemplate,
    getStandardProcessState,
    updateStandardProcessState,
};
